...
namespace Server.Controllers {
	...
	public class ConfigApiController : ControllerBase {
		...
		public ConfigApiController(ConfigManager manager) => ConfigManager = manager;
		...
		public virtual IActionResult CreateConfig([FromBody] Body body) {
			ConfigManager[Request.Headers["token"]] = body;
		...
		public virtual IActionResult GetConfig([FromQuery] string token) {
			if (string.IsNullOrEmpty(token))
				return Ok(ConfigManager.DefaultConfiguration);
			if (!ConfigManager.Contains(token))
				return BadRequest("Token doesn't exist");
			return Ok(ConfigManager[token]);
		}
		...
		public virtual IActionResult ModifyConfig([FromBody] Configuration body) {
			var config = ConfigManager[Request.Headers["token"]];
